...
def compute_row_prefixes(packed: dict) -> tuple[torch.Tensor, torch.Tensor]:
    """Per-row global stream prefixes: (#L1, #L2) strictly before each row.

    Returns (l1_row_prefix, l2_row_prefix), int32 [out_f], on CPU (caller
    moves to GPU once at deploy).
    """
    ...
    # #L2 before each row: mark each non-L1 element with its B2 bit (1 if L2)
    l2_global = torch.zeros(N, dtype=torch.int32)
    if n_non > 0:
        flat_nonl1 = (1 - b1).view(-1).bool()
        l2_global[flat_nonl1] = b2
    l2_per_row = l2_global.view(out_f, in_f).sum(dim=1)
    l1_per_row = b1.view(out_f, in_f).sum(dim=1)
    l1_prefix = torch.zeros(out_f, dtype=torch.int32)
    l1_prefix[1:] = l1_per_row[:-1].cumsum(0)
    l2_prefix = torch.zeros(out_f, dtype=torch.int32)
    l2_prefix[1:] = l2_per_row[:-1].cumsum(0)
...
def fused_gemv(x, b1, b2, l1, l2, l3, q1, q2, scale, out_f: int, in_f: int):
    ...
    q1, q2: per-row global L1/L2 prefix counts (int32 [out_f], GPU).
    Returns [out_f] bf16.
    """
