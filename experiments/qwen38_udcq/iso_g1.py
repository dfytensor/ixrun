# -*- coding: utf-8 -*-
"""Isolation v2: self-contained blob deploy + g1-only capture + graph decode."""
...
def _static_fwd(self, hidden_states, position_embeddings, attention_mask=None,
    ...
    keep = ar < (cum - q_len + 1 + torch.arange(q_len, device=kf.device)[:, None])
...
def build_from_blob():
    ...
    class CpuEmbed(nn.Module):
        def forward(self, ids):
            return blob['embed'][ids.reshape(-1).cpu()].cuda() \
                .view(*ids.shape, -1)

    for name, mod in m.named_modules():
        if 'embed_tokens' in name and type(mod).__name__ == 'Embedding':
            parts = name.split('.')
            parent = m
            for q_ in parts[:-1]:
                parent = getattr(parent, q_)
            setattr(parent, parts[-1], CpuEmbed())
            break
    ...
    print(f'[deploy] {n} linears | resident '
...
def main():
    ...
    print('capture g1+g2 with SHARED pool...', flush=True)
    ...
    hard_reset()
    print('captured g1+g2 (shared pool)', flush=True)
    ...
    for i, tid in enumerate(ids):
        ...
        pos1.fill_(i)
        h_last, log_last = run(emb1, cos1, sin1, pos1)
    h_last, log_last = h_last.clone(), log_last.clone()

    nxt = log_last[:, -1].argmax().item()
    gen = [nxt]
    t = len(ids)
    t0 = time.time()
    for _ in range(29):
    ...
    dt = (time.time() - t0) / 30
    v = json.load(open(r'E:\models\Qwen3.8-27B\vocab.json', encoding='utf-8'))
    inv = {i: s for s, i in v.items()}
    text = ''.join(inv.get(i, '?') for i in gen).replace('Ġ', ' ')
    open(r'C:\Users\Administrator\AppData\Local\Temp\opencode\iso_out.txt', 'w',
         encoding='utf-8').write(text)
    print(f'[{1/dt:.2f} tok/s] g1-only decode done -> iso_out.txt', flush=True)
